...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    ...
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        if (currentFeatureKeys[i] != 'geometry') {
            ...
            if (/\.(mp3|ogg|wav|m4a)$/i.test(fieldValue)) {
                // Gestione file audio
                popupField += (fieldValue != null 
                    ? `<audio controls>
                         <source src="images/${fieldValue.replace(/[\\\/:]/g, '_').trim()}" type="audio/mp3">
				...
				} else if (/\.(mp4|mp3|webm|ogg|avi|mov|flv)$/i.test(fieldValue)) {
